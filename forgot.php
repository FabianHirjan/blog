<?php
	...
	if(isset($_POST['forgotpass'])) {
		...
		if($errMsg == '') {
			try {
				$stmt = $connect->prepare('SELECT password, secretpin, username FROM pdo WHERE secretpin = :secretpin AND username = :username');
				$stmt->execute(array(
					':secretpin' => $secretpin, ':username' => $username
					));
				$data = $stmt->fetch(PDO::FETCH_ASSOC);
				if($secretpin == $data['secretpin'] and $username = $data['username']) {
		...
		<div style=" border: solid 1px #006D9C; " align="left">
			...
			<div style="margin: 15px">
				<form action="" method="post">
					<input type="text" name="username" placeholder="Username" autocomplete="off" class="box"/><br /><br />
